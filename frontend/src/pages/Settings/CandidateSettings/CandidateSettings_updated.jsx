...
import {
    FaUser, FaEnvelope, FaPhone, FaMapMarkerAlt, FaCalendar, FaFileAlt,
    FaLock, FaEye, FaEyeSlash, FaSave, FaUpload, FaCamera, FaEdit, FaDownload
} from 'react-icons/fa';
...
const CandidateSettings = () => {
    ...
    const [currentCvFile, setCurrentCvFile] = useState(''); // Current CV file name
    ...
    const loadProfile = async () => {
        setLoading(true);
        try {
            // API call to get candidate profile
            const response = await axiosInstance.get('/api/user/profile');
            const data = response.data;

            // Map backend response to frontend state (handle both camelCase and PascalCase)
            setProfileData({
            ...
            setAvatarPreview(data.avatar || data.Avatar || '');
            setCurrentCvFile(data.cvFilePath || data.CvFilePath || '');
        } catch (error) {
    ...
    const handleDownloadCV = async () => {
        try {
            const response = await axiosInstance.get('/api/user/download-cv', {
                responseType: 'blob'
            });
                            ...
                            <Form onSubmit={handleProfileSubmit}>
                                ...
                                <Form.Group className="mb-4">
                                    ...
                                    <Form.Text className="text-muted">
                                        ...
                                        {currentCvFile && ' - Chọn file mới để thay thế CV hiện tại'}
                                    </Form.Text>
                                </Form.Group>
