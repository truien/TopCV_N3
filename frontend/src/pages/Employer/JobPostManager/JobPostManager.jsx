...
const JobPostManager = () => {
  ...
  const calculateStatistics = useCallback(
    (postsData) => {
      ...
      const stats = {
        ...
        expired: postsData.filter((p) => new Date(p.applyDeadline) < now)
          .length,
        withPackages: postsData.filter((p) => p.package && p.package.length > 0)
          .length,
        recentPosts: postsData.filter((p) => new Date(p.postDate) > oneWeekAgo)
          .length,
        monthlyPosts: postsData.filter(
          (p) => new Date(p.postDate) > oneMonthAgo
        ).length,
  ...
  const filteredPosts = useMemo(() => {
    ...
    if (dateFilter !== "all") {
      ...
      if (dateFilter !== "all") {
        filtered = filtered.filter(
          (post) => new Date(post.postDate) >= filterDate
        );
      }
    }
  ...
  const handleEditPost = async (post) => {
    ...
    try {
      ...
      setSelectedFields(
        detailData.jobFields?.map((f) => ({ value: f.id, label: f.name })) || []
      );
      setSelectedEmploymentTypes(
        detailData.employmentTypes?.map((t) => ({
          value: t.id,
          label: t.name,
        })) || []
      );
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(
      wb,
      `bai-viet-tuyen-dung-${new Date().toISOString().split("T")[0]}.xlsx`
    );
  ...
  const daysLeft = proInfo?.isPro
    ? Math.ceil(
        (new Date(proInfo.endDate) - new Date()) / (1000 * 60 * 60 * 24)
      )
    : 0;
    ...
    <Container className="mt-4">
      ...
      {proInfo &&
        (proInfo.isPro ? (
          <Alert
            variant="success"
            className={`${styles.proAlert} ${styles.proSuccessAlert}`}
          >
            <div className="d-flex justify-content-between align-items-center">
              <div className={styles.proAlertContent}>
                <FaCrown className={styles.proAlertIcon} />
                <span>
                  Bạn đang sử dụng tài khoản Pro. Còn{" "}
                  <span className={styles.proAlertHighlight}>
                    {daysLeft} ngày
                  </span>{" "}
                  và{" "}
                  <span className={styles.proAlertHighlight}>
                    {proInfo.postsLeft} lượt đăng bài
                  </span>{" "}
                  miễn phí.
                </span>
              </div>
              <Button
                ...
                size="sm"
                className={`${styles.proButton} ${styles.proSuccessButton}`}
                onClick={() => setShowBuyProModal(true)}
              >
                <FaBolt className="me-1" /> Gia hạn Pro
              </Button>
            </div>
          </Alert>
        ) : (
          <Alert
            variant="warning"
            className={`${styles.proAlert} ${styles.proWarningAlert}`}
          >
            <div className="d-flex justify-content-between align-items-center">
              <div className={styles.proAlertContent}>
                <FaBolt className={styles.proAlertIcon} />
                <span>
                  Bạn chưa có tài khoản Pro. Còn{" "}
                  <span className={styles.proAlertHighlight}>
                    {proInfo.postsLeft} lượt đăng bài
                  </span>{" "}
                  miễn phí trong tháng này. Nâng cấp để đăng bài không giới hạn!
                </span>
              </div>
              <Button
                variant="success"
                size="sm"
                className={`${styles.proButton} ${styles.proWarningButton}`}
                onClick={() => setShowBuyProModal(true)}
              >
                <FaCrown className="me-1" /> Mua Pro ngay
              </Button>
            </div>
          </Alert>
        ))}
      ...
      {tab === "reviews" ? (
        ...
        <Table
          ...
          hover
          responsive
          className={`${styles.table} align-middle`}
        >
          <thead>
            <tr>
              <th
              ...
              <th
                className={`${styles.table_th}`}
                style={{ cursor: "pointer" }}
                onClick={() => handleSort("applyDeadline")}
              >
          ...
          <tbody>
            {filteredPosts.map((post) => (
              <tr key={post.id}>
                ...
                <td className={`${styles.table_td}`}>
                  <Badge
                    bg={post.status === "open" ? "success" : "danger"}
                  ...
                  {post.package && post.package.length > 0 ? (
                    <div className={styles.packageGroup}>
                      {post.package.map((pkg, idx) => (
                        ...
                        >
                          {pkg.packageName} -{" "}
                          {isExpired(pkg.endDate) ? "Gia hạn" : "Xem gói"}
                        </Button>
                      ))}
                    </div>
                  ) : (
              ...
              <Form>
                ...
                <Row>
                  ...
                  <Col md={6} className="mb-3">
                    ...
                    <Form.Control
                      ...
                      onChange={(e) =>
                        handleFormChange("salaryRange", e.target.value)
                      }
                ...
                <Row>
                  <Col md={6} className="mb-3">
                    ...
                    <Form.Control
                      ...
                      onChange={(e) =>
                        handleFormChange("applyDeadline", e.target.value)
                      }
                    />
                  </Col>
                  <Col md={6} className="mb-3">
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    ...
                    onChange={(e) =>
                      handleFormChange("interest", e.target.value)
                    }
                  ...
                  <Card>
                    ...
                    <Card.Body>
                      <ProgressBar
                        ...
                        label={`${Math.round(
                          (statistics.withPackages / statistics.total) * 100
                        )}%`}
                        variant="info"
                      />
                    </Card.Body>
                  ...
                  <Alert variant="info">
                    ...
                    <ul className="mb-0">
                      <li>
                        Tổng lượt xem:{" "}
                        <strong>
                          {statistics.totalViews.toLocaleString("vi-VN")}
                        </strong>
                      </li>
                      <li>
                        Bài viết mới trong tháng:{" "}
                        <strong>{statistics.monthlyPosts}</strong>
                      </li>
                      <li>
                        Tỷ lệ thành công:{" "}
                        <strong>
      ...
      <BuyProModal
        onClose={() => setShowBuyProModal(false)}
        show={showBuyProModal}
      />
    </Container>
  );
};
